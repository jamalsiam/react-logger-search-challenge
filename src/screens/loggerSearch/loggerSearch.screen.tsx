...
export const LoggerSearchScreen = () => {
    ...
    const [filteredlogger, setFilteredlogger] = useState<ILoggerTable>({ auditLog: [] });
    ...
    useEffect(() => {
        const onLoadData = async () => {
            ...
            setActionTypeList(uniqueArray(auditLogList.map(itm => itm.actionType)
                .filter(itm => !!itm)));

            setApplicationTypeList(uniqueArray(auditLogList.map(itm => itm.applicationType)
                .filter(itm => !!itm)));
    ...
    return (
        <div>
            ...
            {hasError && <Alert
                ...
                body='Change this and that and try again. Duis mollis, est non commodo.' />}
